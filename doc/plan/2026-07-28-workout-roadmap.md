...
> 狀態：進行中
> 建立日期：2026-07-28
> 最後更新：2026-08-09（**V3 課表實裝與首頁版面最佳化**）
...
> 相關文件：
> - [done/2026-07-31-workout-batch-b.md](done/2026-07-31-workout-batch-b.md)（**批次 B：已完成，人工驗收通過**）
...
| `faadaeb` | **V3 課表**：新增 V3 課表 JSON 檔案，更新提示詞模板 |
| `(即將提交)` | **首頁微調**：並排月曆/年度/備用按鈕，最近 7 天改橫向卡片滾動 |
...
| **批次 B** | 2026-08-08 全數通過（§6.2） |
| **批次 C** | 2026-08-08 全數通過 |
| **V3 課表與首頁** | 2026-08-09 全數通過（§6.3） |
...
### 6.2 批次 B 與 C：全數通過（2026-08-08）

自動驗證全數通過：`vue-tsc` / `eslint`（零警告）/ `vite build`，
外加 esbuild 純函式腳本全過。

**人工驗收已於 2026-08-08 全數通過。**
32 步的逐步操作清單在
[done/2026-07-31-workout-batch-b.md §6.2](done/2026-07-31-workout-batch-b.md)。
...
### 6.3 V3 課表實裝與首頁版面微調（2026-08-09）

因應使用者需求（避開深蹲、跳躍、爬樓梯以保護膝蓋與左腳踝，並因應二尖瓣脫垂與膽結石注意事項，增加生理期彈性舒緩），已匯入 **V3 課表 JSON**。

同時完成 `/workout` 首頁版面大幅微調，節省垂直空間並使資訊更不擁擠：
- 「月曆檢視」與「年度檢視」按鈕並排平分寬度。
- 「今天很忙 (Fallback)」備用課表按鈕並排平分寬度，精簡文案為「⚡ 課表名稱」。
- 「最近 X 天」打卡紀錄從 14 天縮減為 7 天，改為**橫向滾動 (Carousel)**，強制卡片等高，並將標頭日期/星期拆分上下兩行排列。
...
### 批次 B：打卡 checklist 與日曆 — **已完成**

> 2026-07-31 完成實作，2026-08-08 驗收通過。設計與驗收清單見
> [done/2026-07-31-workout-batch-b.md](done/2026-07-31-workout-batch-b.md)。
...
> 2026-08-02 完成實作，2026-08-08 驗收通過。
...
批次 A, B, C 與 schema v2 皆已**全數驗收通過**。

接手者請照這個順序：

1. Drive 備份可視使用者對資料遺失的擔憂程度隨時插隊
2. 進行 **批次 D：接後端**
3. §9.1 的待補資訊隨時可補，其中影片連結不需重匯課表
